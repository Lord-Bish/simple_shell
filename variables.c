...
char* replace_variables(const char* command, int exit_status, int shell_pid) {
    char* result;
    int i = 0, j = 0, len;
    int length;
    ...
    result = (char*)malloc(MAX_INPUT_LENGTH); /* Allocate memory to store the result */
        ...
        if (command[i] == '$') {
            if (i + 1 < length) {
                if (command[i + 1] == '?') {
                    ...
                    strncpy(&result[j], exit_status_str, len);
                    j += len;
                    i += 2;
                } else if (command[i + 1] == '$') {
                    /* Replace "$$" with the shell PID */
                    sprintf(pid_str, "%d", shell_pid);
                    len = strlen(pid_str);
                    strncpy(&result[j], pid_str, len);
                    j += len;
                    i += 2;
                } else {
                    /* Not a recognized variable, copy as is */
                    result[j++] = command[i++];
                }
            } else {
                /* '$' at the end of the command, copy as is */
                result[j++] = command[i++];
            }
        } else {
